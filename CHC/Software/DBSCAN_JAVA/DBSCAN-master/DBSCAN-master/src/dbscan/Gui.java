...
public class Gui {
	...
	public static void makeGui() {
		...
		applydbscan.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				final int numberOfDataPoints = 10;
				...
				if ((str.compareTo(ae.getActionCommand()) == 0)) {
					...
					pta.setText(null);
					trl.clear();
					System.out.println("There are "+hset.size()+" data points");

					trl.addAll(dbscan.applyDbscan());
					Long end= Calendar.getInstance().getTimeInMillis();
					Long durationMin=TimeUnit.MILLISECONDS.toMinutes(end-start);
					Long durationSec=TimeUnit.MILLISECONDS.toSeconds(end-start);

					System.out.println("Took "+durationMin+" minutes");
					System.out.println("Took "+durationSec+" seconds");
					System.out.println("Took "+(end-start)+" millis");

					System.out.println("Found "+ trl.size()+" clusters");
	...
	/*	Random rd=new Random();
		final int numberOfDataPoints = 10;
		for(int i=0;i<numberOfDataPoints;i++) {
			Point p=new Point(i, rd.nextInt(), rd.nextInt(100));
			hset.add(p);
		}*/
		}
...
*/
	...
	public void execute(double[][] values, int minPts, double epsilon) {
		Utility.distanceScores = values;
		this.minpoints = minPts;
		this.tdistance = epsilon;
		
		Random rd=new Random();
		final int numberOfDataPoints = values.length;
		for(int i=0;i<numberOfDataPoints;i++) {
			Point p=new Point(i, i, i);
			hset.add(p);
		}
		...
		String str = "Apply DBscan";
			...
			System.out.println("Found "+ trl.size()+" clusters");
			int amountOfClusteredElements = 0;
			int index1 = 0;
			
			for(List l : trl){
										
				//cluster.add(new Label("Cluster  :" + (index1 + 1)));
				System.out.println("Cluster  :" + (index1 + 1) + " size:"+l.size());
				Iterator<Point> j = l.iterator();
				while (j.hasNext()) {
					Point w = j.next();
//					cluster.add(new Label((w.getX() + "," + w.getY())));
...
//				cluster.add(new Label("***************"));
				System.out.println("***************");
				index1++;
				amountOfClusteredElements += l.size();
			}

			
			card.show(wrap, "Output");
			System.out.println(amountOfClusteredElements + " out of " + values.length + " elements are clustered.");
		//	cluster.revalidate();
